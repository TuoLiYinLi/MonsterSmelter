...
func behavior():
	...
	if(get_target()):
		# 在攻击范围内则进行攻击
		var res:Array = G.grid_manager.check_in_crosss(get_host().grid,get_target(),3)
...
#				var ns = G.path_finder.next_step_A(mt,get_host().grid.index_x, get_host().grid.index_y,get_target().grid.index_x, get_target().grid.index_y)
				var gather = G.grid_manager.check_in_spot(get_target().grid,3)

				var ns = G.path_finder.next_step_multi_A(mt,get_host().grid.index_x, get_host().grid.index_y, gather)
